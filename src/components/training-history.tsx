  ...
  return (
    <section className="rounded-lg border border-slate-800 bg-slate-900 p-4">
      <div className="mb-3 flex items-center justify-between gap-2">
        ...
        <button type="button" disabled={!results.length} onClick={onClear} className="rounded border border-slate-700 px-2 py-1 text-xs hover:bg-slate-800 disabled:opacity-40">{t('resetHistory')}</button>
      </div>
      {results.length === 0 ? (
        ...
        <ul className="divide-y divide-slate-800">
          {results.map((result) => (
            <li key={result.id} className="flex items-center justify-between gap-3 py-2 text-sm">
              ...
              <button type="button" onClick={() => onRestore(result)} className="shrink-0 rounded border border-slate-700 px-2 py-1 text-xs hover:bg-slate-800">{t('restore')}</button>
            </li>
          ))}
        </ul>
      )}
